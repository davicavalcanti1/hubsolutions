    ...
    <div className="min-h-screen flex items-center justify-center bg-background px-4">
      <div className="w-full max-w-sm">
        ...
        <div className="flex gap-2 mb-6">
          {(["company", "account"] as const).map(s => (
            ...
            <form onSubmit={e => { e.preventDefault(); setStep("account"); }}>
              <CardContent className="space-y-4">
                <div className="space-y-1.5">
                  ...
                  <Input id="companyName" placeholder="Minha Clínica" value={companyName}
                    onChange={e => { setCompanyName(e.target.value); setCompanySlug(slugify(e.target.value)); }} required />
                </div>
                <div className="space-y-1.5">
                  <Label htmlFor="companySlug">Identificador (slug)</Label>
                  <Input id="companySlug" placeholder="minha-clinica" value={companySlug}
                    onChange={e => setCompanySlug(slugify(e.target.value))} required />
                  <p className="text-xs text-muted-foreground">Apenas letras, números e traços.</p>
                </div>
                <div className="space-y-1.5">
                  <Label htmlFor="companyEmail">E-mail da empresa <span className="text-muted-foreground">(opcional)</span></Label>
                  <Input id="companyEmail" type="email" placeholder="contato@empresa.com" value={companyEmail}
        ...
        <p className="text-xs text-muted-foreground text-center mt-4">
          Já tem conta?{" "}
          <Link to="/login" className="text-foreground font-medium underline-offset-4 hover:underline">Entrar</Link>
        </p>
      </div>
    </div>
  );
